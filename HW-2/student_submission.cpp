...
#include <unistd.h>
#include <string.h>
//#include <chrono> // comment this out in final submission
#include "raytracer.h"
...
auto image_data = static_cast<int *>(malloc(IMAGE_WIDTH * IMAGE_HEIGHT * sizeof(int) * 3));


void rtxKernel(Checksum &checksumBuffer, Camera camera, std::vector<Sphere> spheres, int start, int end) {
...
int main(int argc, char **argv) {
    ...
    Checksum chckBuffer[THREAD_NUM];

    int threadWidth = width / THREAD_NUM;
    //std::thread threads[THREAD_NUM];
    for(int thread_id = 0; thread_id < THREAD_NUM; thread_id++){
        
        
        int start = threadWidth * thread_id;

        threads[thread_id] = std::thread(rtxKernel, std::ref(chckBuffer[thread_id]), camera, spheres, start, start + threadWidth) ;
    }

    // join the threads
    for(int thread_id = 0; thread_id < THREAD_NUM; thread_id++){
        threads[thread_id].join();
        checksum += chckBuffer[thread_id];
    }
    ...
    //int time_in_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    //std::cout << std::dec << "Operations executed in " << time_in_milliseconds/1000 << " seconds" << std::endl;
    return 0;
}
